...
void order_find()//顺序查找
{
	...
	for (i=0;i<max_words;i++)
	{
		...
		if (dis==0)
		{
			printf("1 %d\n",i+1);//遇到相同的或第一个比待查找的单词大的单词，就要终止查找
			break;
		}
		if (dis>0)//遇到相同的或第一个比待查找的单词大的单词，就要终止查找
		{
			printf("0 %d\n",i+1);
			break;
		}
...
void binary_find(int l,int r)//折半查找
{
	int mid,ans=0;
	while (l<=r)
	{
