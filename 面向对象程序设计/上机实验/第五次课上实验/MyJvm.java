...
public class MyJvm {
    ...
    /*@ public normal_behavior
      ...
      @ assignable heap, heap.elements, heap.size;
      @ requires [4-2]; 条件分支1：加入count数量的object，size未达到上限
      @ requires count >= 0;
      @ requires  heap.size + count <= DEFAULT_CAPACITY;
      @ ensures [4-3]; 调用方法后，size应满足的条件
      @ ensures heap.size == \old(heap.size) + count;
      @ ensures [4-4]; 调用方法后，elements数组应满足仍包含原elements数组全部元素的条件
      @ ensures (\forall int i; 1 <= i && i <= \old(heap.size); heap.elements[i] == \old(heap.elements[i]));
      @ ensures (\forall int i; \old(heap.size) < i && i <= heap.size; heap.elements[i] instanceof MyObject);
      @ ensures (\forall int i; 1 <= i && i <= \old(heap.size); heap.contains(heap.elements[i]));
      @ ensures (\forall int i; \old(heap.size) < i && i <= heap.size; heap.contains(heap.elements[i]) || !heap.elements[i].isReferenced());
      ...
      @ ensures (\forall int i; 1 <= i && i <= heap.size; heap.elements[i] instanceof MyObject);
      @ ensures (\forall int i; 1 <= i && i <= \old(heap.size); heap.contains(heap.elements[i]));
      @ ensures [4-8]; 调用方法后，elements数组应满足: (2)若原elements数组中元素未被引用，则该元素应不包含于elements数组
      @ ensures (\forall int i; \old(heap.size) < i && i <= heap.size; heap.contains(heap.elements[i]) || !heap.elements[i].isReferenced());
      @ ensures (\forall int i; \old(heap.size) < i && i <= heap.size; !heap.contains(heap.elements[i]));
      @*/
